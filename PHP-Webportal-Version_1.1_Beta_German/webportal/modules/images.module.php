...
class images {
	...
	function main() {
		...
		if(isset($_GET['cid']) && ($_GET['cid'] != "") && empty($_GET['sid'])) {
			...
			$result = mysqli_query($connect, "SELECT * FROM images WHERE catid='".htmlspecialchars($_GET['cid'])."' ORDER by id LIMIT ".$limit.",5");
			if(!$result){
				$main = new main();
			...
			$result = mysqli_query($connect, "SELECT id FROM images WHERE catid='".htmlspecialchars($_GET['cid'])."' ORDER by id");
			if(!$result){
				$main = new main();
				$content = $main->error("3","ERROR CONNECTING");
			}
			...
			for($i=1;$i<=$site_sum; ++$i) {
				$sites .= "<a href=\"index.php?id=images&amp;cid=".htmlspecialchars($_GET['cid'])."&amp;page=".$i."\">".$i."</a>&nbsp;";
			}
			$link = "<a href=\"index.php?id=images&amp;sid=images_form&cid=".htmlspecialchars($_GET['cid'])."\">Bild eintragen</a><br>\n<br><hr>\n";
			$render = str_replace("\n","",$content);
			...
			if(isset($result)) {
				$link = "<a href=\"index.php?id=images&amp;cid=".htmlspecialchars($_GET['cid'])."\">Zur Bild-Kategorie</a><br>\n<br>\n";
