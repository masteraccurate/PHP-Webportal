...
class files {
	...
	function main() {
		...
		if(isset($_GET['cid']) && ($_GET['cid'] != "") && empty($_GET['sid'])) {
			...
			$result = mysqli_query($connect, "SELECT * FROM files WHERE catid='".htmlspecialchars($_GET['cid'])."' ORDER by id LIMIT ".$limit.",5");
			if(!$result){
				$main = new main();
			...
			$result = mysqli_query($connect, "SELECT id FROM files WHERE catid='".htmlspecialchars($_GET['cid'])."' ORDER by id");
			if(!$result){
				$main = new main();
				$content = $main->error("3","ERROR CONNECTING");
			}
			...
			for($i=1;$i<=$site_sum; ++$i) {
				$sites .= "<a href=\"index.php?id=files&amp;cid=".htmlspecialchars($_GET['cid'])."&amp;page=".$i."\">".$i."</a>&nbsp;";
			}
			$link = "<a href=\"index.php?id=files&amp;sid=files_form&cid=".htmlspecialchars($_GET['cid'])."\">Datei eintragen</a><br>\n<br><hr>\n";
			$render = str_replace("\n","",$content);
			...
			if(isset($result)) {
				$link = "<a href=\"index.php?id=files&amp;cid=".htmlspecialchars($_GET['cid'])."\">Zur Datei-Kategorie</a><br>\n<br>\n";
