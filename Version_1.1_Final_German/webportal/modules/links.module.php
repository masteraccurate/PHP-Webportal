...
class links {
	...
	function main() {
		...
		if(isset($_GET['cid']) && ($_GET['cid'] != "") && empty($_GET['sid'])) {
			...
			$result = mysqli_query($connect, "SELECT * FROM links WHERE catid='".htmlspecialchars($_GET['cid'], ENT_QUOTES)."' ORDER by id LIMIT ".$limit.",5");
			if(!$result){
			...
			$result = mysqli_query($connect, "SELECT id FROM links WHERE catid='".htmlspecialchars($_GET['cid'], ENT_QUOTES)."' ORDER by id");
			if(!$result){
				$content = $main->error("3","ERROR CONNECTING");
			}
			$sites = "";
			...
			$link = "<a href=\"index.php?id=links&amp;sid=links_form&cid=".htmlspecialchars($_GET['cid'], ENT_QUOTES)."\">Link eintragen</a><br>\n<br><hr>\n";
			$render = str_replace("\n","",$content);
			...
			if(isset($result)) {
				$link = "<a href=\"index.php?id=links&amp;cid=".htmlspecialchars($_GET['cid'], ENT_QUOTES)."\">Zur Link-Kategorie</a><br>\n<br>\n";
