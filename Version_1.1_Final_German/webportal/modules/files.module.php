...
class files {
	...
	function main() {
		...
		if(isset($cid) && ($cid != "") && empty($sid)) {
			...
			$result = $db->query("SELECT * FROM files WHERE catid='".$cid."' ORDER by id LIMIT ".$limit.",5");
			if(!$result){
			...
			$result->close();
			$result = $db->query("SELECT id FROM files WHERE catid='".$cid."' ORDER by id");
			if(!$result){
				$content = $main->error("3","ERROR CONNECTING");
			}
			$sites = "";
			...
			for($i=1;$i<=$site_sum; ++$i) {
				$sites .= "<a href=\"index.php?id=files&amp;cid=".$cid."&amp;page=".$i."\">".$i."</a>&nbsp;";
			}
			$link = "<a href=\"index.php?id=files&amp;sid=files_form&cid=".$cid."\">Datei eintragen</a><br>\n<br><hr>\n";
			$render = str_replace("\n","",$content);
			$render = str_replace("\r","",$render);
			if(isset($_SESSION['loggedin'])) {
				$render = $link.$render;
			}
			$content = $render."Seite: ".$sites."<br><br><a href=\"index.php?id=files\">Zur&uuml;ck zu den Kategorien</a>";
			$db->close();
		} elseif(isset($sid) && isset($cid) && ($sid != "") && ($sid == "files_form") && ($_SESSION['loggedin'] == "1")) {
		...
		} elseif(isset($sid) && ($sid != "") && ($sid == "files_cat_form") && ($_SESSION['loggedin'] == "1")) {
			$template = new template();
			...
			if(isset($result)) {
				$link = "<a href=\"index.php?id=files&amp;cid=".$cid."\">Zur Datei-Kategorie</a><br>\n<br>\n";
				$content = "Datei in Datenbank eingetragen! ".$link;
			} else {
			...
			$result = $db->query($statement);
			if(isset($result)) {
				$link = "<a href=\"index.php?id=files\">Zu den Datei-Kategorien</a><br>\n<br>\n";
				$content = "Datei-Kategorie in Datenbank eingetragen! ".$link;
			} else {
			...
			$result = $db->query("SELECT * FROM files_cat ORDER by catid LIMIT ".$limit.",5");
			if(!$result){
				$content = $main->error("3","ERROR CONNECTING");
			}
			while($row = $result->fetch_array(MYSQLI_BOTH)){
				$content_var = "CatID: ".$row['catid']."<br>\nKategorie: ".$row['category']."<br>\nBenutzername: ".$row['name']."<br>\n Uhrzeit: ".date('Y-m-d H:i:s', $row['datetime'])."<br>\n<a href=\"index.php?id=files&cid=".$row['catid']."\">&Ouml;ffne Kategorie</a><br><hr>\n";
				$content .= $content_var;
			}
			$result->close();
			$result = $db->query("SELECT * FROM files_cat ORDER by catid");
			if(!$result){
				$content = $main->error("3","ERROR CONNECTING");
			}
			$sites = "";
			...
			$link = "<a href=\"index.php?id=files&amp;sid=files_cat_form\">Kategorie eintragen</a><br>\n<br><hr>\n";
			$render = str_replace("\n","",$content);
			$render = str_replace("\r","",$render);
			if(isset($_SESSION['loggedin'])) {
				$render = $link.$render;
